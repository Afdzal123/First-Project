...
app.post('/rides/request', authenticate, authorize(['passenger']), async (req, res) => {
  try {
    const { passengerId, pickup, destination, fare, distance } = req.body;
    if (!passengerId || !pickup || !destination || fare == null || distance == null) {
...
app.patch('/rides/:id/complete', authenticate, async (req, res) => {
  try {
    ...
    if (
      ride.acceptedBy?.toString() !== req.user.userId &&
      req.user.role !== "admin"
    ) {
...
app.get('/analytics/passengers', authenticate, authorize(['admin']), async (req, res) => {
  try {
    const analytics = await db.collection('rides').aggregate([
      { $match: { status: "completed", passengerId: { $ne: null } } },
      {
        $lookup: {
          from: "users",
          localField: "passengerId",
    ...
    res.status(500).json({ error: "Failed to generate passenger analytics", details: err.message });
  }
});
